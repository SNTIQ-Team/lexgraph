...
| `verified_federal_events` | Adjacent official GII states and DIP draft text independently checked against current GII | Lexgraph-generated evidence records. `exact` proves two retrieved official states, not an inferred effective date; `current_text_correspondence` proves only a sufficiently distinctive present-day match, sets `historical_attribution: false`, and asserts neither promulgation nor a patch-level effective date. Buzer is not an input to this public file. |
